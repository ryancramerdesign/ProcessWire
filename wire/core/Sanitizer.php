...
class Sanitizer extends Wire {
	...
	/**
	 * Sanitize SVG markup using SVG Sanitizer
	 *
	 * See: https://github.com/alister-/SVG-Sanitizer
	 *
	 ...
	 *
	 */
	public function svg($pagefile) {
		static $svgSanitizer = null;
		$filename = $pagefile->filename;
		if(!is_file($filename)) throw new WireException("Sanitizer::svg requires a file");
		if(is_null($svgSanitizer)) {
			$svgSanitizer = $this->svgSanitizer();
		}
		return $svgSanitizer->svg($filename, $pagefile->field->svg_sanitizer_whitelist);
	}

	/**
	 * Return a new SVG Sanitizer instance
	 *
	 * See: https://github.com/alister-/SVG-Sanitizer
	 *
	 * @return MarkupSVGSanitizer
	 *
	 */
	public function svgSanitizer() {
		$svgSanitizer = $this->wire('modules')->get('MarkupSVGSanitizer');
		return $svgSanitizer;
	}
