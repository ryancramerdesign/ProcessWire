...
class CommentForm extends Wire implements CommentFormInterface {
	...
	protected $options = array(
		...
		// whether to use custom fields to create Your Name (cite) value
		'yourNameFields' => null,

		// should user fields (Your Name and Your E-mail) be hidden when logged in
		'hideUserFieldsLoggedIn' => 0,
	...
	public function __construct(Page $page, CommentArray $comments, $options = array()) {
		...
		// populate the value of yourNameFields
		if($this->commentsField && is_null($this->options['yourNameFields'])) {
			$this->options['yourNameFields'] = $this->commentsField->yourNameFields;
		}
	}
	...
	public function render() {
		...
		if($user->isLoggedin()) {
			$citeValue = '';
			if($this->options['yourNameFields']){
				foreach($this->options['yourNameFields'] as $yourNameField) {
					$citeValue .= $user->{$this->fields->get($yourNameField)} . ' ';
				}
			}
			else {
				$citeValue = $user->name;
	...
	protected function renderFormNormal($id, $class, $attrs, $labels, $inputValues) {
		...
		$hidden = ($this->user->isLoggedin() && $this->commentsField->hideUserFieldsLoggedIn) ? 'style="display:none"' : '';
		$form .=
		"\n\t<p {$hidden} class='CommentFormCite {$id}_cite'>" .
		...
		"\n\t</p>" .
		"\n\t<p {$hidden} class='CommentFormEmail {$id}_email'>" .
